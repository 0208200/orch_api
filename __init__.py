...
class OrchApi:
    ...
    def _send_request(self, method: str, endpoint: str, id: int = None):
        """
        Send a request to the API.

        Parameters:
        method (str): The HTTP method (e.g., 'GET', 'POST', 'DELETE').
        endpoint (str): The endpoint of the API (e.g., '/odata/Assets').
        id (int, optional): The ID of the organization unit.
        ...
        if id is not None:
            headers["X-UIPATH-OrganizationUnitId"] = f"{id}"

        response = getattr(requests, method.lower())(uri, headers=headers)

        if response.status_code == 200:
            return response.json(), response
        else:
            raise ValueError(
                f"Failed to {method} data. Status code: {str(response.status_code)}. Response: {response.text}"
            )
